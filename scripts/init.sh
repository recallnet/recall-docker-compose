...
utils_image=${project_name}-init-utils
logged_done="true"
...
docker build -t $utils_image -f $current_dir/utils.Dockerfile $current_dir &>> $log_file

log-job-name "Init CometBFT"
run-docker --user root -v $workdir/cometbft:/cometbft $cometbft_image "init --home /cometbft" &>> $log_file
